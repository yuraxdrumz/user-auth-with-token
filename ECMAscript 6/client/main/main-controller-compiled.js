...
(function () {
    angular.module('myApp').controller('mainController', ['$scope', '$http', '$location', 'auth', 'tweet', function ($scope, $http, $location, auth, tweet) {
        ...
        $scope.isLiked = function (tweet) {
            for (var i = 0, len = $scope.tweets[$scope.tweets.indexOf(tweet)].likeFromUser.length; i < len; i++) {
                if ($scope.tweets[$scope.tweets.indexOf(tweet)].likeFromUser[i] == $scope.user._id) return true;else if ($scope.tweets[$scope.tweets.indexOf(tweet)].likeFromUser[i] == undefined) return false;
            }
        };
    }]);
})();
